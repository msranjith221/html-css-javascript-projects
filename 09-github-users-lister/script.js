...
async function searchUser() {
    ...
    if (!username) return alert("Did not enter username");
    ...
    fetchRespositories(userData.repos_url)
    } catch (error) {
        showError();
    }
}

async function fetchRespositories(reposUrl) {
    reposContainer.innerHTML = '<div class="loading-repos">Loading repositories...</div>';

    try {
        const response = await fetch(reposUrl + "?per_page=6");
        // "?per_page=n" fetch up to n items from reposUrl
...
function displayRepos(repos) {
    if (repos.length === 0) {
        reposContainer.innerHTML = '<div class="no-repos">Repository not found</div>';
        return;
    }
    ...
    repos.forEach((repo) => {
        ...
        repoCard.innerHTML = `
          ...
          <div class="repo-meta">
            ...
            <div class="repo-meta-item">
              <i class="fas fa-code-fork"></i> ${repo.fork_count}
            </div>
            <div class="repo-meta-item>
